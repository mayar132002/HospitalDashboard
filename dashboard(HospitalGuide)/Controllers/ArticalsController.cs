    ...
    public class ArticalsController : Controller
    {
        ...
        public ActionResult Create(Artical artical , HttpPostedFileBase ImgUpload)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                /// 
                string fileName = ImgUpload.FileName;
                ImgUpload.SaveAs(Server.MapPath("~//assets//images//") + fileName);
                string path = Path.Combine(Server.MapPath("~//assets//images//"), fileName);
                var url = GetFileUrl(VirtualPathUtility.ToAbsolute("~//assets//images//" + fileName), false);
                ImgUpload.SaveAs(path);
                artical.Artical_Image = url;
                ///
